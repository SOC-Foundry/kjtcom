...
Claude Code is the primary infrastructure executor and supports Gemini CLI on pipeline iterations (Phases 1-5: Acquire, Transcribe, Extract, Normalize, Geocode) when directed. Claude API serves as the primary intelligence engine for complex architectural tasks and infrastructure automation. In v9.48, Claude Code may be directed to lead or support close-out tasks.
...
1. CLAUDE.md MUST be saved to disk in the launch directory BEFORE starting.
2. Verify harness line count: `wc -l CLAUDE.md` - must be >= 200 lines. Harnesses grow, never shrink. (v9.48+)
