...
def _resolve_project_root():
    """Resolve project root via iao_paths if available, else walk up."""
    here = Path(__file__).resolve().parent
    # Try iao_paths from iao-middleware/lib if present
    lib = here.parent / "iao-middleware" / "lib"
    if lib.exists():
        ...
        try:
            ...
            pass
    # Fallback: walk up looking for .iao.json or scripts/
    cur = here
    while cur != cur.parent:
        if (cur / ".iao.json").exists() or (cur / "scripts").is_dir():
            return cur
        cur = cur.parent
    return here.parent
...
def _section_delta_state(iteration):
    ...
    if deltas_script.exists():
        try:
            ...
            pass
    if not ok:
        # Fallback: parse previous build log's Iteration Delta Table
        lines.append("(snapshot generator failed; falling back to previous build log parse)")
        prev_iters = []
        try:
            major, minor = iteration.lstrip("v").split(".")
            prev_iters = [f"v{major}.{int(minor)-i}" for i in range(1, 4)]
        except Exception:
            pass
