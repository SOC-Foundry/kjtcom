"""Fix TripleDB t_any_shows casing.

Reads all TripleDB entities from production, lowercases their
t_any_shows values, and writes back. Same pattern as
fix_calgold_shows_case.py (v8.23 W3).
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Lowercase t_any_shows for TripleDB entities"
    )
    parser.add_argument(
    ...
    query = col.where("t_log_type", "==", "tripledb")
    docs = query.stream()
    ...
    print(f"TripleDB entities scanned. Need update: {len(updates)}, already lowercase: {skipped}")
